...
%% API
%%--------------------------------------------------------------------
-spec start_link(Parent :: pid(), Pool :: poolman:pool(), PoolArgs :: list(), WorkerArgs :: any()) ->
    gen:start_ret().
...
-spec add_worker(Pool :: poolman:pool(), Mod :: module() | pid(), WorkerArgs :: any()) -> {ok, pid()} | {error, any()}.
add_worker(Pool, ModOrPid, WorkerArgs) ->
    gen_server:call(get_server(Pool), {add_worker, undefined, ModOrPid, WorkerArgs}, 5000).

%% @doc add worker to pool, the auto_size must be true
-spec add_worker(Pool :: poolman:pool(), WorkerName :: atom(), ModOrPid :: module() | pid(), WorkerArgs :: any()) ->
    {ok, pid()} | {error, any()}.
...
%% @doc call for poolman_worker
-spec stop_worker(Pool :: poolman:pool(), Id :: integer()) -> ok.
stop_worker(Pool, Id) ->
...
stop_worker(Pid) ->
    poolman_worker:stop(Pid).


-spec stop(Pool :: poolman:pool(), ChildId :: any()) -> ok.
stop(Pool, ChildId) ->
    Info = info(Pool),
    [stop_worker(Pid) || {_, Pid} <- poolman:workers(Pool)],
    gproc_pool:force_delete(poolman:name(Pool)),
...
-spec info(Pool :: poolman:pool()) -> [poolman:info()] | undefined.
info(Pool) ->
...
-spec is_alive(Pool :: poolman:pool()) -> pid() | undefined.
is_alive(Pool) ->
...
-spec get_size(poolman:pool_option()) -> integer().
get_size(PoolArgs) ->
...
init([Parent, Pool, PoolArgs, WorkerArgs]) ->
    ...
    ok = ensure_pool(poolman:name(Pool), PoolType, [{size, PoolSize}, {auto_size, AutoSize}]),
    ok = lists:foreach(
        fun(I) ->
            Name = {Pool, I},
            ensure_pool_worker(poolman:name(Pool), Name, I)
...
handle_call({add_worker, WorkerName, ModOrPid, WorkerArgs}, _From, #state{
    ...
    Result =
        case WorkerName of
            undefined ->
                Name = {Pool, Index},
                ensure_pool_worker(poolman:name(Pool), Name, Index),
                poolman_worker_sup:start_child(Pool, Index, PoolArgs, Args);
            _ ->
                Name = {WorkerName, Index},
                ensure_pool_worker(poolman:name(Pool), Name, Index),
                poolman_worker_sup:start_child(Pool, WorkerName, Index, PoolArgs, Args)
        end,
    case Result of
        {ok, Pid} ->
            {reply, {ok, Pid}, State};
...
-spec ensure_pool(Pool :: poolman:pool(), Type :: poolman:pool_type(), Opts :: poolman:pool_option()) ->
    exists | ok.
...
-spec ensure_pool_worker(Pool :: poolman:pool(), Name :: any(), Slot :: integer()) ->
    ok.
...
-spec get_index(Pool :: poolman:pool()) -> integer().
get_index(Pool) ->
    Workers = poolman:workers(Pool),
...
-spec get_worker_by_id(Pool :: poolman:pool(), Id :: integer()) ->
    undefined | pid().
get_worker_by_id(Pool, Id) ->
    Workers = poolman:workers(Pool),
    find_worker(Id, Workers).

-spec find_worker(Id :: integer(), [poolman:worker()]) -> undefined | pid().
