...
-record(state, {
    name :: {poolcow:pool(), integer()},
    pool :: poolcow:poo_name(),
    mod :: module(),
    child_state :: any()
}).
...
-spec start_link(Pool :: poolcow:pool(), Id :: pos_integer(), Mod :: module(), WorkerArgs :: any()) ->
    gen:start_ret().
...
%%--------------------------------------------------------------------
-spec init({Pool :: poolcow:pool(), Name :: any(), Id :: integer(), Mod :: module(), WorkerArgs :: any()}) ->
    {ok, #state{}} | {stop, any()}.
init({Pool, Name, Id, Mod, WorkerArgs}) ->
    ...
    case Result of
        {ok, ChildState, Timeout} ->
            true = gproc_pool:connect_worker(poolcow:name(Pool), Name),
...
terminate(Reason, #state{mod = Mod, pool = Pool, name = Name, child_state = ChildState}) ->
    gproc_pool:disconnect_worker(poolcow:name(Pool), Name)
        andalso gproc_pool:remove_worker(poolcow:name(Pool), Name)
...
do_init(Id, {con, Mod}, WorkerArgs) ->
    Middleware = poolcow_conn_worker,
