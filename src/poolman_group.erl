...
-spec checkout(GroupName :: poolcow:pool() | pid()) ->
    {ok, Info :: info()} | {error, notfound | any()}.
checkout(WorkerPid) when is_pid(WorkerPid) ->
    gen_server:call(WorkerPid, pick, ?TIMEOUT);
checkout(GroupName) ->
    poolcow:transaction(GroupName,
        fun(Pid) ->
            checkout(Pid)
        end).

%% @doc maksure the auto pool start
-type opt() :: {pool_type, poolcow:pool_type()}.
-spec start(GroupName :: poolcow:pool(), [opt()]) ->
...
start(GroupName, Opts) ->
    ...
    poolcow:start_pool(GroupName, PoolArgs, InitArgs).


-spec ensure_started(GroupName :: poolcow:pool(), [opt()]) -> ok.
ensure_started(GroupName, Opts) ->
    case poolcow:info(GroupName) of
        undefined ->
...
-spec add_worker(GroupName :: poolcow:pool(), ModOrPid :: module() | pid(), WorkerArgs :: any()) ->
    {ok, pid()} | {error, any()}.
add_worker(GroupName, ModOrPid, WorkerArgs) ->
    poolcow_pool:add_worker(GroupName, ModOrPid, WorkerArgs).

-spec add_worker(
    GroupName :: poolcow:pool(),
    WorkerName :: poolcow:pool(),
    ModOrPid :: module() | pid(),
    WorkerArgs :: any()) -> {ok, pid()} | {error, any()}.
add_worker(GroupName, WorkerName, ModOrPid, WorkerArgs) ->
    poolcow_pool:add_worker(GroupName, WorkerName, ModOrPid, WorkerArgs).
