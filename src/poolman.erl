...
pool_spec(Pool, PoolArgs, WorkerArgs) ->
    #{
        id => child_id(Pool),
        start => {poolcow_pool_sup, start_link, [Pool, PoolArgs, WorkerArgs]},
        restart => transient,
        shutdown => 5000,
        type => supervisor,
        modules => [poolcow_pool_sup]
    }.

%% @doc Start the pool supervised by poolcow_sup
...
start_pool(Pool, PoolArgs, WorkerArgs) ->
    ...
    supervisor:start_child(poolcow_sup, Spec).

%% @doc Stop a pool. start by poolcow_sup
-spec stop_pool(Pool :: pool()) -> ok | {error, term()}.
stop_pool(Pool) ->
    ChildId = child_id(Pool),
    poolcow_pool:stop(Pool, ChildId).
...
-spec info(Pool :: poolcow:pool()) -> [info()] | undefined.
info(Pool) ->
    poolcow_pool:info(Pool).
